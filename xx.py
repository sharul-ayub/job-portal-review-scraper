...
async def _crawl_once(url: str, browser_config: BrowserConfig, crawl_config: CrawlerRunConfig):
...
async def scrape_reviews():
    ...
    # Primary mode: reuse profile from setup_profile.py
    primary_browser_config = BrowserConfig(
        headless=False,
    ...
    # Fallback mode: fresh non-managed session using same profile path
    fallback_browser_config = BrowserConfig(
        headless=False,
    ...
    for page_num in range(total_pages):
        ...
        for attempt in range(1, 3):
            ...
            # Attempt 1: primary profile mode
            result = await _crawl_once(url, primary_browser_config, crawl_config)

            await asyncio.sleep(PAGE_STABILIZE_DELAY_SECONDS)

            if not result.success:
                last_error = f"primary crawl failed: {result.error_message}"
                print(f"Page {page_idx}: primary crawl failed on attempt {attempt}: {result.error_message}")
            else:
                ...
                if html:
                    try:
                        ...
                        print(f"Page {page_idx}: primary parse failed on attempt {attempt}: {exc}")
                else:
                    last_error = "primary returned empty HTML"
                    print(f"Page {page_idx}: primary returned empty HTML on attempt {attempt}")

            # Fallback for same attempt
            fallback_result = await _crawl_once(url, fallback_browser_config, crawl_config)
            await asyncio.sleep(PAGE_STABILIZE_DELAY_SECONDS)
            if not fallback_result.success:
                last_error = f"fallback crawl failed: {fallback_result.error_message}"
                print(f"Page {page_idx}: fallback crawl failed on attempt {attempt}: {fallback_result.error_message}")
                continue

            html = getattr(fallback_result, "html", "") or ""
            if not html:
                last_error = "fallback returned empty HTML"
                print(f"Page {page_idx}: fallback returned empty HTML on attempt {attempt}")
                continue
            ...
            try:
                ...
                print(f"Page {page_idx}: fallback parse failed on attempt {attempt}: {exc}")
